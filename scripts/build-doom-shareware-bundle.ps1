...
try {
  ...
  # The shareware installer does not ship DEFAULT.CFG; SETUP.EXE normally
  # creates it. Launching DOOM.EXE directly without one leaves the DMX sound
  # devices disabled. Device 2 selects the original AdLib OPL music renderer,
  # while device 3 keeps Sound Blaster digital effects. No substitute
  # soundtrack or external MIDI files are needed.
  $doomConfig = @"
mouse_sensitivity 5
sfx_volume 8
music_volume 15
...
"@
  [System.IO.File]::WriteAllText(
    (Join-Path $gameDirectory "DEFAULT.CFG"),
    $doomConfig,
